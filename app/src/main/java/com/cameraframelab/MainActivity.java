...
public final class MainActivity extends Activity implements CameraRecorder.Listener {
    ...
    private void runNextSuiteProfile() {
        ...
        if (suiteIndex >= suiteQueue.size()) {
            suiteRunning = false;
            suiteButton.setText("Run automatic suite");
            status.setText("Suite complete. Reports: Downloads/CameraFrameLab · Videos: Movies/CameraFrameLab");
            updateButtons();
            return;
        }
            ...
            try {
                ...
                main.post(() -> {
                    ...
                    if (suiteRunning) {
                        ...
                        status.setText("Saved. Reports: Downloads/CameraFrameLab · Video: Movies/CameraFrameLab");
                    }
                });
            } catch (Throwable t) {
                main.post(() -> {
