...
public final class ReportWriter {
    ...
    public static ReportResult write(Context context, CameraRecorder.SessionResult session, TestProfile profile,
        ...
        Uri summary = writeDownload(context, session.baseName + "_summary.json", "application/json", root.toString(2));
        Uri cameraCsv = writeDownload(context, session.baseName + "_camera.csv", "text/csv", cameraCsv(camera));
        Uri encoderCsv = writeDownload(context, session.baseName + "_encoder.csv", "text/csv", encoderCsv(encoder));
        Uri thermalCsv = writeDownload(context, session.baseName + "_thermal.csv", "text/csv", thermalCsv(thermal));
    ...
    private static Uri writeDownload(Context context, String name, String mime, String body) throws Exception {
